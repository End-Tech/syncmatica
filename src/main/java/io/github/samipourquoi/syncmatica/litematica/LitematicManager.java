...
public class LitematicManager {
	...
	public ServerPlacement syncmaticFromSchematic(SchematicPlacement schem) {
		...
		byte[] hash = null;
		try {
			hash = SyncmaticaUtil.createChecksum(new FileInputStream(placementFile));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		ServerPlacement placement = new ServerPlacement(UUID.randomUUID(), placementFile.getName(), hash);
